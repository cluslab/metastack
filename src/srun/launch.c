	...
	slurm_opt_t *opt_local, bool use_all_cpus, srun_job_t *job)
{
	...
	if (opt_local->cpus_per_dcu) {
		...
		/* Like cpus_per_task, imply --exact */
		if (srun_opt->whole)
			info("Ignoring --whole since --cpus-per-dcu used");
		else if (!srun_opt->exact)
			verbose("Implicitly setting --exact, because --cpus-per-dcu given.");
		srun_opt->exact = true;
	}
#endif
	...
	if (opt_local->cpus_per_npu) {
		...
		/* Like cpus_per_task, imply --exact */
		if (srun_opt->whole)
			info("Ignoring --whole since --cpus-per-npu used");
		else if (!srun_opt->exact)
			verbose("Implicitly setting --exact, because --cpus-per-npu given.");
		srun_opt->exact = true;
	}
#endif
...
#ifdef __METASTACK_NEW_GRES_DCU
	} else if (opt_local->cpus_per_dcu) {
		if (opt_local->dcus) {
			int dcus_per_step;

			dcus_per_step = _parse_gpu_request(opt_local->dcus);
			step_req->cpu_count = dcus_per_step *
				opt_local->cpus_per_dcu;
		} else if (opt_local->dcus_per_node) {
			int dcus_per_node;

			dcus_per_node =
				_parse_gpu_request(opt_local->dcus_per_node);
			/* Use a minimum value for requested cpus */
			step_req->cpu_count = opt_local->min_nodes *
				dcus_per_node * opt_local->cpus_per_dcu;
		} else if (opt_local->tres_per_task &&
			   (pos = xstrstr(opt_local->tres_per_task,
					  "gres/dcu:"))) {
			pos += 9; /* Don't include "gres/dcu:" */
			step_req->cpu_count = opt_local->ntasks *
					      _parse_gpu_request(pos) *
					      opt_local->cpus_per_dcu;
		} else if (add_tres) {
			int rc = SLURM_SUCCESS;
			uint64_t dcus_per_node = 0;
			char *save_ptr = NULL;

			while (slurm_option_get_tres_per_tres(
				       opt_local->gres, "dcu",
				       &dcus_per_node, &save_ptr,
				       &rc));
			if (rc != SLURM_SUCCESS) {
				/* Failed to parse, error already logged */
				slurm_free_job_step_create_request_msg(
			...
			/*
			 * Same math as dcus_per_node
			 *
			 * If dcus_per_node == 0, then the step did not request
			 * dcus, but the step may still inherit dcus from the
			 * job. This math will set requested cpus to zero in
			 * case dcus_per_node == 0.
			 */
			step_req->cpu_count = opt_local->min_nodes *
				dcus_per_node * opt_local->cpus_per_dcu;
		} else if (opt_local->dcus_per_socket) {
			/*
			 * dcus_per_socket is not fully supported for steps and
			 * does not affect the dcus allocated to the step:
			 * slurmctld may inherit gres from the job.
			 */
		}
#endif
#ifdef __METASTACK_NEW_GRES_NPU
	} else if (opt_local->cpus_per_npu) {
		if (opt_local->npus) {
			int npus_per_step;

			npus_per_step = _parse_gpu_request(opt_local->npus);
			step_req->cpu_count = npus_per_step *
				opt_local->cpus_per_npu;
		} else if (opt_local->npus_per_node) {
			int npus_per_node;

			npus_per_node =
				_parse_gpu_request(opt_local->npus_per_node);
			/* Use a minimum value for requested cpus */
			step_req->cpu_count = opt_local->min_nodes *
				npus_per_node * opt_local->cpus_per_npu;
		} else if (opt_local->tres_per_task &&
			   (pos = xstrstr(opt_local->tres_per_task,
					  "gres/npu:"))) {
			pos += 9; /* Don't include "gres/npu:" */
			step_req->cpu_count = opt_local->ntasks *
					      _parse_gpu_request(pos) *
					      opt_local->cpus_per_npu;
		} else if (add_tres) {
			int rc = SLURM_SUCCESS;
			uint64_t npus_per_node = 0;
			char *save_ptr = NULL;

			while (slurm_option_get_tres_per_tres(
				       opt_local->gres, "npu",
				       &npus_per_node, &save_ptr,
				       &rc));
			if (rc != SLURM_SUCCESS) {
				/* Failed to parse, error already logged */
				slurm_free_job_step_create_request_msg(
			...
			/*
			 * Same math as npus_per_node
			 *
			 * If npus_per_node == 0, then the step did not request
			 * npus, but the step may still inherit npus from the
			 * job. This math will set requested cpus to zero in
			 * case npus_per_node == 0.
			 */
			step_req->cpu_count = opt_local->min_nodes *
				npus_per_node * opt_local->cpus_per_npu;
		} else if (opt_local->npus_per_socket) {
			/*
			 * npus_per_socket is not fully supported for steps and
			 * does not affect the npus allocated to the step:
			 * slurmctld may inherit gres from the job.
			 */
		}
#endif
	...
	if (opt_local->nodelist) {
		...
		if (!hl) {
			...
			if ((opt_local->distribution & SLURM_DIST_STATE_BASE) != SLURM_DIST_ARBITRARY) {
				hostlist_uniq(hl);
			}

			step_req->node_list = hostlist_ranged_string_xmalloc(hl);
		}
		hostlist_destroy(hl);
	} else {
		step_req->node_list = xstrdup(opt_local->nodelist);
	...
	if (!opt_local->tres_bind &&
		...
		if (opt_local->ntasks_per_tres != NO_VAL)
			xstrfmtcat(opt_local->tres_bind, "gres/dcu:single:%d",
				   opt_local->ntasks_per_tres);
		else
			xstrfmtcat(opt_local->tres_bind, "gres/dcu:single:%d",
	...
	if (!opt_local->tres_bind &&
		...
		if (opt_local->ntasks_per_tres != NO_VAL)
			xstrfmtcat(opt_local->tres_bind, "gres/npu:single:%d",
				   opt_local->ntasks_per_tres);
		else
			xstrfmtcat(opt_local->tres_bind, "gres/npu:single:%d",
