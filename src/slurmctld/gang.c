...
/* add_job_to_cores2, Add job's core_bitmap to full-length core_bitmap, 
 * if job_state==CG, then only add cores in job's cg node.
 * 
 * Replace the function add_job_to_cores(src\common\job_resources.c).
 * job_resources.h cann't call sturct job_record_t declared in slurmctld.h
 * because of cross include. 
 * 
