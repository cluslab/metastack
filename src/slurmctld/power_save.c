...
static void _do_idle_work(time_t now, bitstr_t *avoid_node_bitmap,
	...
	list_itr_t * part_itr = list_iterator_create(part_list);
	...
	for (i = 0; (tmp_node_ptr = next_node(&i)); i++) {
...
#ifdef __METASTACK_NEW_SUSPEND_KEEP_IDLE
		exc_part_list = list_create(NULL);
#endif
...
#ifdef __METASTACK_NEW_SUSPEND_KEEP_IDLE
				list_append(exc_part_list, part_ptr);
#endif
			} else {
				exc_node_bitmap =
