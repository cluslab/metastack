			     ...
			     update_part_msg_t *part_msg_ptr)
{
	...
	for (i = 0; i < argc; i++) {
		...
		if (xstrncasecmp(tag, "PartitionName", MAX(taglen, 2)) == 0) {
			...
			if (xstrncasecmp(val, "NO", MAX(vallen, 1)) == 0)
				part_msg_ptr->meta_flags |= PART_METAFLAG_RBN_CLR;
			else if (xstrncasecmp(val, "YES", MAX(vallen, 1)) == 0)
				part_msg_ptr->meta_flags |= PART_METAFLAG_RBN;
			else {
				exit_code = 1;
				error("Invalid input: %s", argv[i]);
				error("Acceptable RBN values are YES and NO");
				return SLURM_ERROR;
			}
			(*update_cnt_ptr)++;
		}
			...
			if (xstrncasecmp(val, "NO", MAX(vallen, 1)) == 0)
				...
				exit_code = 1;
				error("Invalid input: %s", argv[i]);
				error("Acceptable LLS values are YES and NO");
				return SLURM_ERROR;
			}
			(*update_cnt_ptr)++;
		}
