...
/*Analysis of sinfo environment variables*/
static int _parse_sinfo_format_extend (char* format_long, char* cmp, int* field_size, 
												 bool *right_justify) 
{
...
/*Restore full fields for abbreviated characters 
 *and parse the format set by the environment variable.*/
										 ...
										 bool *right_justify, bool format_all)
{
	...
	char *field_tmp = NULL;
	if (field[0] == 'a') {
		field_tmp = xstrdup("available");
	} else if (field[0] == 'A') {
		field_tmp = xstrdup("nodeai");
	} else if (field[0] == 'b') {
		field_tmp = xstrdup("features_act");
	} else if (field[0] == 'B') {
		field_tmp = xstrdup("maxcpuspernode");
	} else if (field[0] == 'c') {
		field_tmp = xstrdup("cpus");
	} else if (field[0] == 'C') {
		field_tmp = xstrdup("cpusstate");
	} else if (field[0] == 'd') {
		field_tmp = xstrdup("disk");
	} else if (field[0] == 'D') {
		field_tmp = xstrdup("nodes");	
	} else if (field[0] == 'e') {
		field_tmp = xstrdup("freemem");
	} else if (field[0] == 'E') {
		field_tmp = xstrdup("reason");
	} else if (field[0] == 'f') {
		field_tmp = xstrdup("features");
	} else if (field[0] == 'F') {
		field_tmp = xstrdup("nodeaiot");
	} else if (field[0] == 'g') {
		field_tmp = xstrdup("groups");
	} else if (field[0] == 'G') {
		field_tmp = xstrdup("gres");
	} else if (field[0] == 'h') {
		field_tmp = xstrdup("oversubscribe");
	} else if (field[0] == 'H') {
		field_tmp = xstrdup("timestamp");
	} else if (field[0] == 'I') {
		field_tmp = xstrdup("priorityjobfactor");
	} else if (field[0] == 'l') {
		field_tmp = xstrdup("time");
	} else if (field[0] == 'L') {
		field_tmp = xstrdup("defaulttime");
	} else if (field[0] == 'm') {
		field_tmp = xstrdup("memory");
	} else if (field[0] == 'M') {
		field_tmp = xstrdup("preemptmode");
	} else if (field[0] == 'n') {
		field_tmp = xstrdup("nodehost");
	} else if (field[0] == 'N') {
		field_tmp = xstrdup("nodelist");
	} else if (field[0] == 'o') {
		field_tmp = xstrdup("nodeaddr");
	} else if (field[0] == 'O') {
		field_tmp = xstrdup("cpusload");
	} else if (field[0] == 'p') {
		field_tmp = xstrdup("prioritytier");
	} else if (field[0] == 'P') {
		field_tmp = xstrdup("partition");
	} else if (field[0] == 'r') {
		field_tmp = xstrdup("root");
	} else if (field[0] == 'R') {
		field_tmp = xstrdup("partitionname");
	} else if (field[0] == 's') {
		field_tmp = xstrdup("size");
	} else if (field[0] == 'S') {
		field_tmp = xstrdup("allocnodes");
	} else if (field[0] == 't') {
		field_tmp = xstrdup("statecompact");
	} else if (field[0] == 'T') {
		field_tmp = xstrdup("statelong");
	} else if (field[0] == 'u') {
		field_tmp = xstrdup("user");
	} else if (field[0] == 'U') {
		field_tmp = xstrdup("userlong");
	} else if (field[0] == 'v') {
		field_tmp = xstrdup("version");
	} else if (field[0] == 'V') {
		field_tmp = xstrdup("cluster");
	} else if (field[0] == 'w') {
		field_tmp = xstrdup("weight");
	} else if (field[0] == 'X') {
		...
		field_tmp = xstrdup("socketcorethread");
	} else if (format_all) {
		/*do nothing*/
	} else {
		fprintf(stderr, "Invalid sinfo format specification: %c\n",
		field[0] );
	}
	if(field_tmp != NULL) 
	    token = field_tmp;
		
	if ((env_sinfo = getenv("SINFO_EXTEND")) && token) {
		//*field_size = abs(*field_size);
		char *format_sinfo = xstrdup(env_sinfo);
		_parse_sinfo_format_extend(format_sinfo, token, field_size, right_justify);
        if(all_flag)
			 *field_size = 0;
		if (format_sinfo)
			xfree(format_sinfo);
...
static int _parse_long_format (char* format_long)
{
	...
	while (token) {
		_parse_long_token( token, sep, &field_size, &right_justify,
				   &suffix);
#ifdef __METASTACK_OPT_PRINT_COMMAND
       /*Parse the sinfo environment variable and assign the print format*/
	   field_size = abs(field_size);
	   if ((env_sinfo = getenv("SINFO_EXTEND"))) {
			//field_size = abs(field_size);
			char *format_sinfo = xstrdup(env_sinfo);
        ...
        } else if (!xstrcasecmp(token, "SuspendKeepIdle")) {
			params.match_flags.suspend_idle_flag = true;
			format_add_suspendidlenum(params.format_list,
						 field_size,
						 right_justify,
						 suffix );
#endif
...
#ifdef __METASTACK_NEW_SUSPEND_KEEP_IDLE
	printf("suspend_idle_flag   = %s\n",
			params.match_flags.suspend_idle_flag ?
			"true" : "false");
#endif
