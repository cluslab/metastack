			 ...
			 bool right_justify, char *suffix)
{
	char id[FORMAT_STRING_SIZE];
	if (sinfo_data) {
		if ((sinfo_data->part_info && sinfo_data->part_info->suspend_idle == NO_VAL) ||
					sinfo_data->part_info == NULL)
			_print_str("n/a", width, right_justify, true);
		else {
			_build_min_max_32_string(id, FORMAT_STRING_SIZE,
					      sinfo_data->part_info->suspend_idle,
					      sinfo_data->part_info->suspend_idle,
					      true, true);
			_print_str(id, width, right_justify, true);
		}
	} else
		_print_str("SUSPEND_KEEP_IDLE", width, right_justify, true);
