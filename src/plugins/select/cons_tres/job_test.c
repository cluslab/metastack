...
static void _set_gpu_defaults(job_record_t *job_ptr)
{
	...
	uint64_t last_cpu_per_gpu = NO_VAL64;
	uint64_t last_mem_per_gpu = NO_VAL64;
#else
...
/*
*The bitmap of available nodes is selected using job information and the number
*of available resources of nodes

* IN: job_ptr      - pointer to the job requesting resources
* IN: node_map - bitmap of available nodes / bitmap of selected nodes
* IN: min_nodes    - minimum number of nodes required
* IN: max_nodes    - maximum number of nodes requested
* IN: avail_res_array    - Number of resources available on each node
*/
			...
			uint32_t min_nodes,	uint32_t max_nodes,	avail_res_t **avail_res_array, bitstr_t *idle_bitmap)
{
	...
	int *cpu_size = NULL, node_count = 0 ;
	...
	for (i = i_first; i <= i_last; i++) {
		...
		* that is bigger than our max cpu count. */
		...
		//Gets the maximum avail cpu number in a node
		if(avail_res_array[i]->avail_cpus > max_node_cpu)
			max_node_cpu = avail_res_array[i]->avail_cpus;
	}

	cpu_size = xcalloc(max_node_cpu + 1, sizeof(int));
	...
	/*Select the nodes that can be used in the -N+-n scenario*/
				   ...
				   resv_exc_t *resv_exc_ptr)
{
	...
	if (enable_high_throughput &&
		    (topo_eval.max_nodes == 1 || details_ptr->num_tasks == 1 || details_ptr->num_tasks == details_ptr->ntasks_per_node ) &&
            (topo_eval.req_nodes == 1) &&
            (topo_eval.job_ptr->details && !topo_eval.job_ptr->details->req_node_bitmap) &&
            !(topo_eval.cr_type & CR_LLN) &&
            (topo_eval.job_ptr->part_ptr &&
#ifdef __METASTACK_NEW_HETPART_SUPPORT
			!(topo_eval.job_ptr->part_ptr->meta_flags & PART_METAFLAG_HETPART) &&
#endif
            !(topo_eval.job_ptr->part_ptr->flags & PART_FLAG_LLN))) {
            log_flag(SELECT_TYPE, "enable_high_throughput:%d, topo_eval.job_ptr->details->min_cpus: %d, topo_eval.rep_nodes: %d", enable_high_throughput, topo_eval.job_ptr->details->min_cpus, topo_eval.req_nodes);
            topo_eval.avail_res_array = 
				_get_res_avail2(topo_eval.job_ptr, topo_eval.node_map, 
	...
	job_ptr->resv_stage != RESV_STAGE_SUBMIT_OR_BACKFILL){
		...
		if((details_ptr->min_nodes == details_ptr->max_nodes) && details_ptr->num_tasks &&
			...
			(details_ptr->mc_ptr->ntasks_per_socket ==  INFINITE16)){
				...
				node_bitmap_t = _job_choose_nodes(job_ptr, node_bitmap, topo_eval.min_nodes,  topo_eval.max_nodes, topo_eval.avail_res_array, job_ptr->resv_bitmap);
				FREE_NULL_BITMAP(job_ptr->resv_bitmap);
		     ...
		     bool qos_preemptor, bool preempt_mode)
{
	...
	if (license_list) {
		...
		int license_rc = 0;
		if(job_ptr->lic_main_schedule){
			license_rc = sched_license_job_test_with_list(job_ptr, time(NULL),
									true, license_list);
		} else {
			license_rc = license_job_test_with_list(job_ptr, time(NULL),
									true, license_list);
		}
#else
...
alloc_job:
	...
	if (xstrstr(job_ptr->tres_per_job, "dcu") || xstrstr(job_ptr->tres_per_node, "dcu") || xstrstr(job_ptr->tres_per_socket, "dcu") || xstrstr(job_ptr->tres_per_task, "dcu"))
...
#ifdef __METASTACK_BUG_BACKFILL_ENDLESS_LOOP
		time_t time_window = 30;
#else
		int time_window = 30;
#endif
		time_t end_time = 0;
