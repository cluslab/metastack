				...
				time_t *period_start)
{
	...
	for(i=0;i<cnt;i++) {
		...
		event.cluster_nodes = KCIResultGetColumnValue(result,i,EVENT_REQ_CNODES);//3
		event.node_name = KCIResultGetColumnValue(result,i,EVENT_REQ_NODE);//2
		event.period_end = KCIResultGetColumnValue(result,i,EVENT_REQ_END);//1
		event.period_start = KCIResultGetColumnValue(result,i,EVENT_REQ_START);//0
		event.reason = KCIResultGetColumnValue(result,i,EVENT_REQ_REASON);//4
		event.reason_uid = KCIResultGetColumnValue(result,i,EVENT_REQ_REASON_UID);//5
		event.state = KCIResultGetColumnValue(result,i,EVENT_REQ_STATE);//6
		event.tres_str = KCIResultGetColumnValue(result,i,EVENT_REQ_TRES);//7
	...
	kingbase_conn_t *kingbase_conn, slurmdb_archive_rec_t *arch_rec)
{
	...
	xassert(kingbase_conn->rollback);
