/*****************************************************************************\
 *  as_kingbase_federation.c - functions dealing with federations.
 *****************************************************************************
 ...
 */
static int _remove_all_clusters_from_fed(kingbase_conn_t *kingbase_conn,
...
static int _remove_clusters_from_fed(kingbase_conn_t *kingbase_conn, List clusters)
{
	int   rc    = SLURM_SUCCESS;
	char *query = NULL;
	char *name  = NULL;
...
static int _add_clusters_to_fed(kingbase_conn_t *kingbase_conn, List clusters,
				const char *fed)
{
	int   rc      = SLURM_SUCCESS;
	char *query   = NULL;
	...
	while ((name = list_next(itr))) {
		int id;
		if ((rc = as_kingbase_get_fed_cluster_id(kingbase_conn, name, fed,
						      last_id, &id)))
			goto end_it;
		last_id = id;
	 ...
	 * Also note that kingbase evaluates from left to right and uses the
...
static int _assign_clusters_to_federation(kingbase_conn_t *kingbase_conn,
					  ...
					  List cluster_list)
{
	...
	if (clear_clusters &&
	    (rc = _remove_all_clusters_from_fed(kingbase_conn, federation,
						add_list)))
		goto end_it;
	if (!clear_clusters &&
	    list_count(rem_list) &&
	    (rc = _remove_clusters_from_fed(kingbase_conn, rem_list)))
		goto end_it;
	if (list_count(add_list) &&
	    (rc = _add_clusters_to_fed(kingbase_conn, add_list, federation)))
		goto end_it;
				    ...
				    List federation_list)
{
	...
	while ((object = list_next(itr))) {
		...
		if (object->cluster_list &&
		    _assign_clusters_to_federation(kingbase_conn, object->name,
						   object->cluster_list)) {
			xfree(cols);
			xfree(vals);
			xfree(extra);
...
extern List as_kingbase_get_federations(kingbase_conn_t *kingbase_conn, uid_t uid,
...
empty:
    ...
    for(i = 0 ; i < rows ; ++i){
        slurmdb_cluster_cond_t clus_cond;
 		...
 		fed->name  = xstrdup(KCIResultGetColumnValue(result,i,FED_REQ_NAME));
 		fed->flags = slurm_atoul(KCIResultGetColumnValue(result,i,FED_REQ_FLAGS));
		...
		clus_cond.federation_list = list_create(xfree_ptr);
 		...
 		tmp_list = as_kingbase_get_clusters(kingbase_conn, uid, &clus_cond);
...
extern List as_kingbase_modify_federations(
				...
				slurmdb_federation_rec_t *fed)
{
	...
	KCIResultDealloc(result);
	if (fed->cluster_list &&
	    (_assign_clusters_to_federation(kingbase_conn, object,
					    fed->cluster_list))) {
		xfree(vals);
		xfree(name_char);
		xfree(query);
	...
	if (!list_count(ret_list)) {
		errno = SLURM_NO_CHANGE_IN_DATA;
		DB_DEBUG(FEDR, kingbase_conn->conn,
	...
	if (vals) {
		...
		rc = modify_common(kingbase_conn, DBD_MODIFY_FEDERATIONS, now,
...
extern List as_kingbase_remove_federations(kingbase_conn_t *kingbase_conn, uint32_t uid,
					slurmdb_federation_cond_t *fed_cond)
{
	List ret_list = NULL;
	int rc = SLURM_SUCCESS;
    ...
    for( i = 0 ; i < rows ; i++){
		...
		if ((rc = _remove_all_clusters_from_fed(kingbase_conn, object,
							NULL)))
			break;
		...
		if ((rc = remove_common(kingbase_conn, DBD_REMOVE_FEDERATIONS, now,
