/*****************************************************************************\
 *  as_kingbase_rollup.c - functions for rolling up data for associations
 *                   and machines from the as_kingbase storage.
 *****************************************************************************
...
static int _process_purge(kingbase_conn_t *kingbase_conn,
			  char *cluster_name,
...
static void _setup_cluster_tres_usage(kingbase_conn_t *kingbase_conn,
				      char *cluster_name,
...
static int _process_cluster_usage(kingbase_conn_t *kingbase_conn,
				  char *cluster_name,
				  time_t curr_start, time_t curr_end,
				  time_t now, local_cluster_usage_t *c_usage)
{
	...
	while ((loc_tres = list_next(itr))) {
		_setup_cluster_tres_usage(kingbase_conn, cluster_name,
	...
	xstrfmtcat(query,
		   ...
		   "mod_time=%ld, count=new.count, "
		   "alloc_secs=new.alloc_secs, "
		   "down_secs=new.down_secs, "
		   "pdown_secs=new.pdown_secs, "
		   "idle_secs=new.idle_secs, "
		   "over_secs=new.over_secs, "
		   "plan_secs=new.plan_secs",
		   now);
...
static local_cluster_usage_t *_setup_cluster_usage(kingbase_conn_t *kingbase_conn,
						   char *cluster_name,
						   time_t curr_start,
						   time_t curr_end,
						   ...
						   int dims)
{
	...
	for(int i = 0; i < KCIResultGetRowCount(result); i++) {
		time_t row_start = slurm_atoul(KCIResultGetColumnValue(result, i, EVENT_REQ_START));
		time_t row_end = slurm_atoul(KCIResultGetColumnValue(result, i, EVENT_REQ_END));
		uint16_t state = slurm_atoul(KCIResultGetColumnValue(result, i, EVENT_REQ_STATE));
		...
		/* this means we are a cluster registration
		   entry */
		char *temp = KCIResultGetColumnValue(result, i, EVENT_REQ_NAME);
		if (*temp == '\0') {
			local_cluster_usage_t *loc_c_usage;
			...
			_add_tres_2_list(loc_c_usage->loc_tres,
					 KCIResultGetColumnValue(result, i, EVENT_REQ_TRES), seconds);
		...
		while ((loc_r_usage = list_next(r_itr))) {
			...
			if (hostlist_find_dims(loc_r_usage->hl,
					       KCIResultGetColumnValue(result, i, EVENT_REQ_NAME), dims)
			    < 0)
				continue;
			...
			_add_tres_time_2_list(loc_tres,
					      KCIResultGetColumnValue(result, i, EVENT_REQ_TRES),
					      loc_r_usage->flags &
					      RESERVE_FLAG_MAINT ?
		...
		while ((loc_c_usage = list_next(d_itr))) {
			...
			/*      "wasn't responding", */
			/*      KCIResultGetColumnValue(result, i, EVENT_REQ_NAME), */
...
extern int _setup_resv_usage(kingbase_conn_t *kingbase_conn,
			     char *cluster_name,
			     time_t curr_start,
			     time_t curr_end,
			     List resv_usage_list,
			     int dims)
{
	 ...
	 */
	for(int i = 0; i < KCIResultGetRowCount(result); i++) {
		time_t row_start = slurm_atoul(KCIResultGetColumnValue(result, i, RESV_REQ_START));
		time_t row_end = slurm_atoul(KCIResultGetColumnValue(result, i, RESV_REQ_END));
		int unused;
		int resv_seconds;
		...
		if (row_start >= curr_start) {
			...
			unused = slurm_atoul(KCIResultGetColumnValue(result, i, RESV_REQ_UNUSED));
		...
		r_usage->flags = slurm_atoul(KCIResultGetColumnValue(result, i, RESV_REQ_FLAGS));
		r_usage->id = slurm_atoul(KCIResultGetColumnValue(result, i, RESV_REQ_ID));

		r_usage->local_assocs = list_create(xfree_ptr);
		slurm_addto_char_list(r_usage->local_assocs,
				      KCIResultGetColumnValue(result, i, RESV_REQ_ASSOCS));
		r_usage->loc_tres =
			list_create(_destroy_local_tres_usage);

		_add_tres_2_list(r_usage->loc_tres,
				 KCIResultGetColumnValue(result, i, RESV_REQ_TRES), resv_seconds);
		...
		r_usage->hl = hostlist_create_dims(KCIResultGetColumnValue(result, i, RESV_REQ_NODES), dims);
...
extern int as_kingbase_hourly_rollup(kingbase_conn_t *kingbase_conn,
				  char *cluster_name,
...
/* 	info("begin end %s", slurm_ctime2(&curr_end)); */
	...
	while (curr_start < end) {
		...
		DB_DEBUG(DB_USAGE, kingbase_conn->conn,
...
/* 		info("end %s", slurm_ctime2(&curr_end)); */
		...
		if ((rc = _setup_resv_usage(kingbase_conn, cluster_name,
					    curr_start, curr_end,
					    resv_usage_list, dims))
		    != SLURM_SUCCESS)
			goto end_it;

		c_usage = _setup_cluster_usage(kingbase_conn, cluster_name,
					       curr_start, curr_end,
					       resv_usage_list,
					       cluster_down_list,
					       dims);
		...
		for(int i = 0; i < KCIResultGetRowCount(result); i++) {
			//uint32_t job_id = slurm_atoul(KCIResultGetColumnValue(result, i, JOB_REQ_JOBID));
			uint32_t assoc_id = slurm_atoul(KCIResultGetColumnValue(result, i, JOB_REQ_ASSOCID));
			uint32_t wckey_id = slurm_atoul(KCIResultGetColumnValue(result, i, JOB_REQ_WCKEYID));
			uint32_t array_pending =
				slurm_atoul(KCIResultGetColumnValue(result, i, JOB_REQ_ARRAY_PENDING));
			uint32_t resv_id = slurm_atoul(KCIResultGetColumnValue(result, i, JOB_REQ_RESVID));
			time_t row_eligible = slurm_atoul(KCIResultGetColumnValue(result, i, JOB_REQ_ELG));
			time_t row_start = slurm_atoul(KCIResultGetColumnValue(result, i, JOB_REQ_START));
			time_t row_end = slurm_atoul(KCIResultGetColumnValue(result, i, JOB_REQ_END));
			uint32_t row_rcpu = slurm_atoul(KCIResultGetColumnValue(result, i, JOB_REQ_RCPU));
			List loc_tres = NULL;
			int loc_seconds = 0;
			...
			if (slurm_atoul(KCIResultGetColumnValue(result, i, JOB_REQ_SUSPENDED))) {
				...
				query = xstrdup_printf(
					...
					suspend_table,
					curr_end, curr_start,
					KCIResultGetColumnValue(result, i, JOB_REQ_DB_INX));
		...
		calc_cluster:
			...
			_add_tres_time_2_list(loc_tres, KCIResultGetColumnValue(result, i, JOB_REQ_TRES),
					      TIME_ALLOC, seconds,
					      suspend_seconds, 0);
			if (w_usage)
				_add_tres_time_2_list(w_usage->loc_tres,
						      KCIResultGetColumnValue(result, i, JOB_REQ_TRES),
						      TIME_ALLOC, seconds,
						      suspend_seconds, 0);
		...
		*/
		...
		if (c_usage) {
			...
			if ((rc = _process_cluster_usage(
				     kingbase_conn, cluster_name, curr_start,
...
/* 	info("stop end %s", slurm_ctime2(&curr_end)); */
	...
	if (rc == SLURM_SUCCESS) {
		if (kingbase_db_commit(kingbase_conn)) {
			...
			rc = _process_purge(kingbase_conn, cluster_name,
					    archive_data, SLURMDB_PURGE_HOURS);
	}

	return rc;
}
extern int as_kingbase_nonhour_rollup(kingbase_conn_t *kingbase_conn,
				   bool run_month,
				   char *cluster_name,
				   time_t start, time_t end,
				   uint16_t archive_data)
...
/* 		info("end %s", slurm_ctime2(&curr_end)); */
		...
		query = xstrdup_printf(
			...
			now, now ,curr_start,
			cluster_name, run_month ? assoc_day_table : assoc_hour_table, curr_end, curr_start,
			cluster_name, run_month ? assoc_month_table : assoc_day_table,
			now
			);
		...
		*/
		...
		xstrfmtcat(query,
				...
				"(select * from cte2) on duplicate key update "
				"mod_time=%ld, count=cte2.CPU, "
				"alloc_secs=cte2.ASUM, down_secs=cte2.DSUM, "
				"pdown_secs=cte2.PDSUM, idle_secs=cte2.ISUM, "
				"over_secs=cte2.OSUM, plan_secs=cte2.PSUM; ",
				now, now, curr_start,
				cluster_name, run_month ? cluster_day_table : cluster_hour_table,
				curr_end, curr_start,
				cluster_name, run_month ? cluster_month_table : cluster_day_table,
				now
			   );
		if (track_wckey) {
			// xstrfmtcat(query,
			...
			xstrfmtcat(query,
				   "with cte3 as ("
					...
					now, now, curr_start,
					cluster_name, run_month ? wckey_day_table : wckey_hour_table,
					curr_end, curr_start,
					cluster_name, run_month ? wckey_month_table : wckey_day_table,
					now
				   );
		}
		DB_DEBUG(DB_USAGE, kingbase_conn->conn, "query\n%s", query);
...
/* 	info("stop end %s", slurm_ctime2(&curr_end)); */
	...
	/* go check to see if we archive and purge */
	rc = _process_purge(kingbase_conn, cluster_name, archive_data,
