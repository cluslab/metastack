...
/*
		...
		char *tpn = NULL;
		char *hjnl = NULL;
		hostlist_t slurm_job_hl;
		hostlist_t hl;
		tpn = getenvp(*env, PMIXP_JOB_NODES_ENV_DEP);
		if (tpn){
			slurm_job_hl = hostlist_create(tpn);
			hl = hostlist_create("");
			i = 0;
			while(i<_pmixp_job_info.nnodes){
				for(j=0; j<_pmixp_job_info.nnodes; j++){
					if(job->het_job_tids[j][0] == k){
						k += _pmixp_job_info.task_cnts[j];
						i++;
						char *node_s = hostlist_nth(slurm_job_hl, j);
			...
			if (hl) {
				hjnl = hostlist_ranged_string_xmalloc(hl);
				env_array_overwrite(env, PMIXP_STEP_NODES_ENV, hjnl);
				env_array_overwrite(env, PMIXP_JOB_NODES_ENV, hjnl);
				xfree(hjnl);
			}
			hostlist_destroy(hl);
			hostlist_destroy(slurm_job_hl);
		}
*/
#endif
	} else {
