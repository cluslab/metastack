...
void _event_print(uint64_t count, uint64_t  *start, uint64_t  *end, abnormal_event_type_t flag) {
	...
	case LOW_CPU_UTIL:
		...
		event_desc = event_str[LOW_CPU_UTIL];
		break;
	case PROC_ABNORMAL:
		printf("Display of PROCESS exception events\n");
		event_desc = event_str[PROC_ABNORMAL];
		break;
	case NODE_ABNORMAL:
		printf("Display of Node exception events\n");
		event_desc = event_str[NODE_ABNORMAL];
		break;
	}
	for (int i = 0; i < MIN(count, JOBACCTINFO_START_END_ARRAY_SIZE); i++) {
		/* Handling access to the circular buffer */
		...
		char start_str[100] = {0}, end_str[100] = {0};
		...
		printf("\n%s between %s and %s\n", event_desc, start_str, end_str);
	}	
}

void print_for_briebf(jobacctinfo_t *total_jobacct,uint32_t job_id, uint32_t step_id)
{
	...
	if((params.opt_event == 1) && total_jobacct) {
	   ...
	   if(step_id != -4) {
			...
			if(total_jobacct) {
				...
				print_resource.all_task_mem_tmp_max = total_jobacct->mem_step_max;
				print_resource.all_task_mem_tmp_min = total_jobacct->mem_step_min;

				print_resource.all_task_vmem_tmp_max = total_jobacct->vmem_step_max;
				print_resource.all_task_vmem_tmp_min = total_jobacct->vmem_step_min;
				/* mem real */		
				print_resource.all_task_mem_tmp = total_jobacct->mem_step;
				/* vmem real */	
				print_resource.all_task_vmem_tmp = total_jobacct->vmem_step;
		   }
	   }
   }

}
void print_max_min_detail(uint32_t step_id)
{
	...
	if(step_id != -4) {
		if(print_resource.all_task_vmem_tmp_max >=0) {
			...
			convert_num_unit(print_resource.all_task_mem_tmp, outbuf_tmp,
					sizeof(outbuf_tmp), UNIT_NONE, UNIT_MEGA,
					params.convert_flags);
			/* vmem usage conversion */
			convert_num_unit(print_resource.all_task_vmem_tmp, vmem_outbuf_tmp,
					sizeof(vmem_outbuf_tmp), UNIT_NONE, UNIT_MEGA,
					params.convert_flags);

			convert_num_unit(print_resource.all_task_mem_tmp_max, outbuf_tmp_max,
					sizeof(outbuf_tmp_max), UNIT_NONE, UNIT_MEGA,
					params.convert_flags);
			convert_num_unit(print_resource.all_task_mem_tmp_min, outbuf_tmp_min,
					sizeof(outbuf_tmp_min), UNIT_NONE, UNIT_MEGA,
					params.convert_flags);

			convert_num_unit(print_resource.all_task_vmem_tmp_max, vmem_outbuf_tmp_max,
					sizeof(vmem_outbuf_tmp_max), UNIT_NONE, UNIT_MEGA,
					params.convert_flags);
			convert_num_unit(print_resource.all_task_vmem_tmp_min, vmem_outbuf_tmp_min,
	     ...
	     uint32_t req_cpufreq_gov, uint16_t use_protocol_ver)
{
...
#ifdef __METASTACK_NEW_LOAD_ABNORMAL
 	print_for_briebf(total_jobacct, step.step_id.job_id, step.step_id.step_id);
#endif
	if (total_jobacct) {
	...
	if((params.opt_event == 1) && tot_tasks ) {
		print_max_min_detail(step.step_id.step_id);
	}
#endif
#ifdef __METASTACK_NEW_LOAD_ABNORMAL
	if(params.opt_event != 1)
