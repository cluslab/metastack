...
static void _rpc_clean_bb(slurm_msg_t *msg)
{
	...
	for (uint32_t dataset_idx = 0; dataset_idx < dataset_count; dataset_idx++) {
		...
		if (rc == 0 && recycle_task_id > 0) {
			...
			error("BB-----提交数据集%u回收任务失败, return code=%d", dataset_id, rc);
			goto cleanup;
		}
	}
	debug("BB-----job_id=%u, (%u 个)回收任务提交成功", job_id, dataset_count);
	/* 等待回收任务完成 */
	debug("BB-----job_id=%u, 开始等待所有回收任务完成");
	for (uint32_t dataset_idx = 0; dataset_idx < dataset_count; dataset_idx++) {
		...
		if (rc == 0) {
			...
			error("BB-----回收任务%u失败, return code=%d", recycle_task_id, rc);
			goto cleanup;
		}
	}
	...
	for (uint32_t dataset_idx = 0; dataset_idx < dataset_count; dataset_idx++) {
		...
		if (rc == 0) {
			...
			error("BB-----删除数据集规则%u失败, return code=%d", dataset_id, rc);
			goto cleanup;
		}
	}
