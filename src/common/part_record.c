...
extern void set_standby_nodes_flag(part_record_t *part_ptr)
{
	...
	if (part_ptr->standby_nodes->offline_node_state > 0 &&
			 ((part_ptr->standby_nodes->job_age_range && part_ptr->standby_nodes->job_age_range[1] > 0) ||
			 (part_ptr->standby_nodes->job_count_range && part_ptr->standby_nodes->job_count_range[1] > 0))) {
		part_ptr->standby_nodes->enable_for_job  = true;
		part_ptr->standby_nodes->enable_for_node = true;
	} else if (part_ptr->standby_nodes->offline_node_state > 0) {
		part_ptr->standby_nodes->enable_for_job  = false;
		part_ptr->standby_nodes->enable_for_node = true;
	} else if ((part_ptr->standby_nodes->job_age_range && part_ptr->standby_nodes->job_age_range[1] > 0) ||
				(part_ptr->standby_nodes->job_count_range && part_ptr->standby_nodes->job_count_range[1] > 0)) {
		part_ptr->standby_nodes->enable_for_job  = true;
...
void standby_nodes_free(standby_nodes_t *standby_nodes)
{
	if (standby_nodes) {
		...
		xfree(standby_nodes->job_age_range);
		xfree(standby_nodes->job_count_range);
...
#ifdef __METASTACK_NEW_SUSPEND_KEEP_IDLE
		safe_unpack32(&part_ptr->suspend_idle, buffer);
#endif
#ifdef __METASTACK_NEW_AUTO_SUPPLEMENT_AVAIL_NODES
		if (!part_ptr->standby_nodes) {
			_init_part_record_standby_nodes(part_ptr);
		}
		safe_unpackstr(&part_ptr->standby_nodes->borrowed_nodes, buffer);
#endif
#ifdef __METASTACK_PART_PRIORITY_WEIGHT
		if (!part_ptr->priority_params) {
			_init_part_record_priority_params(part_ptr);
		}
		safe_unpack16(&part_ptr->priority_params->priority_favor_small,  buffer);
		safe_unpack32(&part_ptr->priority_params->priority_weight_age,   buffer);
		safe_unpack32(&part_ptr->priority_params->priority_weight_assoc, buffer);
		safe_unpack32(&part_ptr->priority_params->priority_weight_fs,    buffer);
		safe_unpack32(&part_ptr->priority_params->priority_weight_js,    buffer);
		safe_unpack32(&part_ptr->priority_params->priority_weight_part,  buffer);
		safe_unpack32(&part_ptr->priority_params->priority_weight_qos,   buffer);
		safe_unpackstr(&part_ptr->priority_params->priority_weight_tres, buffer);
#endif
	} else if (protocol_version >= SLURM_24_05_PROTOCOL_VERSION) {
		safe_unpack32(&part_ptr->cpu_bind, buffer);
		safe_unpackstr(&part_ptr->name, buffer);
