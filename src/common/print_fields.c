...
/* Split the string and assign the parsed value to the print function, 
 * which can be overridden by the specified format.*/
extern int _parse_env_format_extend (char* format_long, print_field_t *field) 
{
		...
		if (value) {
            ...
            if(field->right_format) {
				...
				if(tmp_len == len) {
					...
					memcpy(&temp_char, value + (len - tmp_len)*sizeof(char), tmp_len + 1);
				}
				if (len > abs_len)
					temp_char[0] = '+';
			} else {
