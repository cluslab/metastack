...
typedef struct {
	...
	uint16_t *job_age_range;   /* age range for pending job */
	uint16_t *job_count_range; /* count range for pending job */
	uint32_t pending_jobs;  /* the number of pending jobs due to insufficient resources */
	uint32_t max_pending_age;  /* the maximum age of pending jobs due to insufficient resources */
	uint32_t nodes_need_for_job;  /* the number of nodes to be used for the resource job */
} standby_nodes_t;
#endif
