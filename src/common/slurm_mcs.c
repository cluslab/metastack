...
typedef struct slurm_mcs_ops {
	...
	int (*check_user)		(slurmdb_user_rec_t *user, char *mcs_label);
#endif
} slurm_mcs_ops_t;
...
#ifdef __METASTACK_OPT_PART_VISIBLE
extern int mcs_g_check_mcs_label_user(slurmdb_user_rec_t *user, char *mcs_label)
{
	if (slurm_mcs_init() < 0)
		return 0;

	return (int)(*(ops.check_user))(user, mcs_label);
}
#endif
