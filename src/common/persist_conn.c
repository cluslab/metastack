...
static bool _conn_readable1(persist_conn_t *persist_conn)
{
	...
	while (!(*persist_conn->shutdown)) {
		...
		if (rc == 0) {
			...
			persist_conn->flags |= PERSIST_FLAG_TIMEOUT;
			return false;
		}
		if (ufds.revents & POLLHUP) {
			...
			persist_conn->flags |= PERSIST_FLAG_DONT_UPDATE_CLUSTER;
						...
						int thread_loc, void *arg)
{
	...
	/**
	 * fix bug 104294
	 * Ensure that the allocated thread_loc is unique
	 */
	slurm_mutex_lock(&thread_count_lock);
#endif
	...
	if (thread_loc < 0)
#ifdef __METASTACK_BUG_USERADD_RECONFIG_CTLD_CRASH
		thread_loc = slurm_persist_conn_wait_for_thread_loc(true);
#else
...
#ifdef __METASTACK_BUG_USERADD_RECONFIG_CTLD_CRASH
	/**fix bug 104294
	 * Ensure thread_id is set before unlocking, so slurm_persist_conn_recv_server_fini 
	 * waits for thread exit before freeing service_conn.
	 */
	slurm_mutex_unlock(&thread_count_lock);
#endif
}
...
#ifdef __METASTACK_BUG_USERADD_RECONFIG_CTLD_CRASH
extern int slurm_persist_conn_wait_for_thread_loc(bool locked)
#else
...
extern int slurm_persist_conn_open1(persist_conn_t *persist_conn)
{
	...
	if (slurm_send_node_msg1(persist_conn->fd, &req_msg) < 0) {
		...
		buffer = _slurm_persist_recv_msg1(persist_conn, false);
...
{
	slurm_persist_conn_close(persist_conn);
	return slurm_persist_conn_open1(persist_conn);
}
#endif
				  ...
				  buf_t *buffer)
{
	...
	rc = slurm_persist_conn_writeable1(persist_conn);
	if (rc == -1) {
	re_open:
		...
		if (persist_conn->flags & PERSIST_FLAG_RECONNECT) {
			slurm_persist_conn_reopen1(persist_conn);
			rc = slurm_persist_conn_writeable1(persist_conn);
		} else
			return SLURM_ERROR;
	}
				      ...
				      bool reopen)
{
	...
	* If the connection is closed, modify the connection flags to avoid reconnection.
	*/
	msg_read = tls_g_recv1(persist_conn->tls_conn, &nw_size,
			      sizeof(nw_size), &connection_closed);
	...
	while (msg_size > offset) {
		...
		* If the connection is closed, modify the connection flags to avoid reconnection.
		*/
		msg_read = tls_g_recv1(persist_conn->tls_conn, (msg + offset),
				      (msg_size - offset), &connection_closed);
		if (connection_closed) {
			persist_conn->flags &= (~PERSIST_FLAG_RECONNECT);
			persist_conn->flags |= PERSIST_FLAG_DONT_UPDATE_CLUSTER;
			slurm_persist_conn_close(persist_conn);
