...
struct plugin_args *pyxis_args_register(spank_t sp, struct plugin_config config)
{
	...
	bool sharefs_support = config.sharefs_support;

	for (int i = 0; spank_opts[i].name != NULL; ++i) {
		if ((strcmp(spank_opts[i].name, "container-name") == 0) && (sharefs_support == true)) {
			spank_opts[i].usage = "[pyxis] name to use for saving and loading the container on the host. "
						"The current container data directory is in a shared path and the --container-image option is disabled. "
						"Please use the command `enroot create` to create containers with the name 'pyxis_<NAME>'."
						"If necessary, use the command `enroot remove` to remove the container with the name 'pyxis_<NAME>' after the job is completed.";
		}
		if ((strcmp(spank_opts[i].name, "container-image") != 0) || (!sharefs_support)) {
