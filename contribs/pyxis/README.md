...
## Configuration
The plugin can be configured using the `plugstack.conf` file, which is located in `/opt/gridview/slurm/etc/`.

```ini
# /opt/gridview/slurm/etc/plugstack.conf

# Pyxis runtime path (default: "/run/pyxis")
required /opt/gridview/slurm/lib/slurm/spank_pyxis.so runtime_path=/run/pyxis

# Whether or not to execute the entrypoint specified in the image (default: "false")
required /opt/gridview/slurm/lib/slurm/spank_pyxis.so execute_entrypoint=true

# Container creation scope that affects the prefix of the container name (default: "SCOPE_GLOBAL")
required /opt/gridview/slurm/lib/slurm/spank_pyxis.so container_scope=SCOPE_JOB

# Whether or not the sbatch/salloc command can use the options provided by pyxis (default: "true")
required /opt/gridview/slurm/lib/slurm/spank_pyxis.so sbatch_support=false

# Whether the container data directory is located on the share, if true disable the --container-image option (default: "false")
required /opt/gridview/slurm/lib/slurm/spank_pyxis.so sharefs_support=true
```
