...
int pyxis_config_parse(struct plugin_config *config, int ac, char **av)
{
	...
	for (int i = 0; i < ac; ++i) {
		if (strncmp("runtime_path=", av[i], 13) == 0) {
...
#ifdef METASTACK_NEW_PYXIS_PARASTORAGE_SUPPORT
		} else if (strncmp("sharefs_support=", av[i], 16) == 0) {
			optarg = av[i] + 16;
			if (optarg == NULL) {
				ret = -1;
			} else {
			...
			if (ret < 0) {
				slurm_error("pyxis: sharefs_support: invalid value: %s", optarg);
				return (-1);
			}
			config->sharefs_support = ret;
#endif
		} else {
